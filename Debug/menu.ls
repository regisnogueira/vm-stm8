 ...
 104                     ; 29     disableInterrupts();
 107  0024 9b            sim
 109                     ; 30     EXTI_SetExtIntSensitivity(EXTI_PORT_GPIOC, EXTI_SENSITIVITY_FALL_ONLY);
 112  0025 ae0202        	ldw	x,#514
 113  0028 cd0000        	call	_EXTI_SetExtIntSensitivity
 115                     ; 31     EXTI_SetTLISensitivity(EXTI_TLISENSITIVITY_FALL_ONLY);
 117  002b 4f            	clr	a
 118  002c cd0000        	call	_EXTI_SetTLISensitivity
 120                     ; 32     ITC_SetSoftwarePriority(ITC_IRQ_PORTC , ITC_PRIORITYLEVEL_3);
 122  002f ae0503        	ldw	x,#1283
 123  0032 cd0000        	call	_ITC_SetSoftwarePriority
 125                     ; 33     enableInterrupts();
 128  0035 9a            rim
 130                     ; 35     menu.idx = 0;
 133  0036 3f09          	clr	_menu
 134                     ; 36     menu.value = get_menu_value(menu.idx);
 136  0038 4f            	clr	a
 137  0039 ad03          	call	_get_menu_value
 139  003b b70a          	ld	_menu+1,a
 140                     ; 38 }
 143  003d 81            	ret
 178                     ; 40 uint8_t get_menu_value(uint8_t idx)
 178                     ; 41 {
 179                     	switch	.text
 180  003e               _get_menu_value:
 184                     ; 43     return (uint8_t)get_eeprom_data(idx);
 186  003e cd0000        	call	_get_eeprom_data
 188  0041 9f            	ld	a,xl
 191  0042 81            	ret
 217                     ; 49 void set_menu_value(void)
 217                     ; 50 {
 218                     	switch	.text
 219  0043               _set_menu_value:
 223                     ; 52     set_eeprom_data(menu.value, menu.idx);
 ...
 245                     ; 56 }
 248  005f 81            	ret
 273                     ; 58 void tmr_menu(void)
 273                     ; 59 {
 274                     	switch	.text
 275  0060               _tmr_menu:
 279                     ; 61     _decrement(menu.tmr);
 ...
 288  006b               L16:
 289                     ; 62     _decrement(btn[BTN_SET_IDX].debounce);
 ...
 298  0076               L36:
 299                     ; 63     _decrement(btn[BTN_INC_IDX].debounce);
 ...
 308  0081               L56:
 309                     ; 64     _decrement(btn[BTN_DEC_IDX].debounce);
 ...
 319                     ; 66 }
 322  008c 81            	ret
 348                     ; 68 static void inc_option(void)
 348                     ; 69 {
 349                     	switch	.text
 350  008d               L17_inc_option:
 354                     ; 71     if (++menu.idx > PAR_COUNT)
 ...
 468                     ; 95 }
 471  00bb 81            	ret
 495                     ; 97 static void dec_value(void)
 495                     ; 98 {
 496                     	switch	.text
 497  00bc               L731_dec_value:
 501                     ; 100     if (menu.value) {
 ...
 510  00c4               L151:
 511                     ; 103         menu.value = MAX_VALUE;
 513  00c4 3563000a      	mov	_menu+1,#99
 514  00c8               L351:
 515                     ; 106 }
 518  00c8 81            	ret
 545                     ; 108 void interrupt_buttons(void)
 545                     ; 109 {
 546                     	switch	.text
 ...
 693  0147               L512:
 694                     ; 144     if (btn[BTN_INC_IDX].status == BTN_STAT_PRESSED) {
 696  0147 b606          	ld	a,_btn+6
 697  0149 a101          	cp	a,#1
 698  014b 260e          	jrne	L122
 699                     ; 145         btn[BTN_INC_IDX].status = BTN_STAT_FREE;
 701  014d 3f06          	clr	_btn+6
 702                     ; 146         if (!menu.edit)
 ...
 717  015b               L122:
 718                     ; 151     if (btn[BTN_DEC_IDX].status == BTN_STAT_PRESSED) {
 720  015b b603          	ld	a,_btn+3
 721  015d a101          	cp	a,#1
 722  015f 260e          	jrne	L722
 723                     ; 152         btn[BTN_DEC_IDX].status = BTN_STAT_FREE;
 725  0161 3f03          	clr	_btn+3
 726                     ; 153         if (!menu.edit)
 ...
 751                     ; 159         btn[BTN_SET_IDX].debounce = BTN_DEBOUNCE;
 ...
 813  0000               L742_dp:
 814  0000 00            	dc.b	0
 853                     ; 178 void task_menu(void)
 853                     ; 179 {
 854                     	switch	.text
 855  01c0               _task_menu:
 859                     ; 183     process_button();
 ...
 870  01c7 81            	ret
 871  01c8               L762:
 872                     ; 188     if (menu.write)
 874  01c8 3d0c          	tnz	_menu+3
 875  01ca 2703          	jreq	L172
 876                     ; 189         set_menu_value();
 878  01cc cd0043        	call	_set_menu_value
 880  01cf               L172:
 881                     ; 191     dp = (menu.edit) ? (uint8_t)(dp^1u) : 0u;
 883  01cf 3d0b          	tnz	_menu+2
 884  01d1 2706          	jreq	L63
 885  01d3 b600          	ld	a,L742_dp
 886  01d5 a801          	xor	a,#1
 887  01d7 2001          	jra	L04
 888  01d9               L63:
 889  01d9 4f            	clr	a
 890  01da               L04:
 891  01da b700          	ld	L742_dp,a
 892                     ; 193     set_display_option(par[menu.idx].option, dp);
 894  01dc b600          	ld	a,L742_dp
 895  01de 97            	ld	xl,a
 ...
 904                     ; 194     set_display_value(menu.value, par[menu.idx].dp);
