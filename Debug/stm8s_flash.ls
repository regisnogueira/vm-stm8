 ...
 817  00e1 7b02          	ld	a,(OFST-2,sp)
 818  00e3 43            	cpl	a
 819  00e4 1101          	cp	a,(OFST-3,sp)
 820  00e6 2617          	jrne	L133
 821                     ; 323       res_value = (uint16_t)((uint16_t)value_optbyte << 8);
 823  00e8 7b01          	ld	a,(OFST-3,sp)
 824  00ea 5f            	clrw	x
 825  00eb 97            	ld	xl,a
 826  00ec 4f            	clr	a
...
1427                     ; 576   UNUSED(FLASH_MemType);
1430  0186 200e          	jra	L175
1431  0188               L765:
1432                     ; 579     flagstatus = (uint8_t)(FLASH->IAPSR & (FLASH_IAPSR_EOP | FLASH_IAPSR_WR_PG_DIS));
1434  0188 c6505f        	ld	a,20575
1435  018b a405          	and	a,#5
1436  018d 6b03          	ld	(OFST+0,sp),a
1438                     ; 580     timeout--;
1440  018f 1e01          	ldw	x,(OFST-2,sp)
1441  0191 1d0001        	subw	x,#1
1442  0194 1f01          	ldw	(OFST-2,sp),x
1444  0196               L175:
1445                     ; 577   while((flagstatus == 0x00) && (timeout != 0x00))
1447  0196 0d03          	tnz	(OFST+0,sp)
1448  0198 2604          	jrne	L575
1450  019a 1e01          	ldw	x,(OFST-2,sp)
1451  019c 26ea          	jrne	L765
1452  019e               L575:
1453                     ; 584   if(timeout == 0x00 )
1455  019e 1e01          	ldw	x,(OFST-2,sp)
1456  01a0 2604          	jrne	L775
1457                     ; 586     flagstatus = FLASH_STATUS_TIMEOUT;
1459  01a2 a602          	ld	a,#2
1460  01a4 6b03          	ld	(OFST+0,sp),a
1462  01a6               L775:
1463                     ; 589   return((FLASH_Status_TypeDef)flagstatus);
1465  01a6 7b03          	ld	a,(OFST+0,sp)
1468  01a8 5b03          	addw	sp,#3
1469  01aa 81            	ret
1532                     ; 599 IN_RAM(void FLASH_EraseBlock(uint16_t BlockNum, FLASH_MemType_TypeDef FLASH_MemType))
1532                     ; 600 {
1533                     	switch	.text
1534  01ab               _FLASH_EraseBlock:
1536  01ab 89            	pushw	x
1537  01ac 5206          	subw	sp,#6
1538       00000006      OFST:	set	6
1541                     ; 601   uint32_t startaddress = 0;
1543                     ; 611   assert_param(IS_MEMORY_TYPE_OK(FLASH_MemType));
1545                     ; 612   if(FLASH_MemType == FLASH_MEMTYPE_PROG)
1547  01ae 7b0b          	ld	a,(OFST+5,sp)
1548  01b0 a1fd          	cp	a,#253
1549  01b2 260c          	jrne	L336
1550                     ; 614     assert_param(IS_FLASH_PROG_BLOCK_NUMBER_OK(BlockNum));
1552                     ; 615     startaddress = FLASH_PROG_START_PHYSICAL_ADDRESS;
1554  01b4 ae8000        	ldw	x,#32768
1555  01b7 1f05          	ldw	(OFST-1,sp),x
1556  01b9 ae0000        	ldw	x,#0
1557  01bc 1f03          	ldw	(OFST-3,sp),x
1560  01be 200a          	jra	L536
1561  01c0               L336:
1562                     ; 619     assert_param(IS_FLASH_DATA_BLOCK_NUMBER_OK(BlockNum));
1564                     ; 620     startaddress = FLASH_DATA_START_PHYSICAL_ADDRESS;
1566  01c0 ae4000        	ldw	x,#16384
1567  01c3 1f05          	ldw	(OFST-1,sp),x
1568  01c5 ae0000        	ldw	x,#0
1569  01c8 1f03          	ldw	(OFST-3,sp),x
1571  01ca               L536:
1572                     ; 628     pwFlash = (PointerAttr uint32_t *)(MemoryAddressCast)(startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE));
1574  01ca 1e07          	ldw	x,(OFST+1,sp)
1575  01cc a640          	ld	a,#64
1576  01ce cd0000        	call	c_cmulx
1578  01d1 96            	ldw	x,sp
1579  01d2 1c0003        	addw	x,#OFST-3
1580  01d5 cd0000        	call	c_ladd
1582  01d8 be02          	ldw	x,c_lreg+2
1583  01da 1f01          	ldw	(OFST-5,sp),x
1585                     ; 632   FLASH->CR2 |= FLASH_CR2_ERASE;
1587  01dc 721a505b      	bset	20571,#5
1588                     ; 633   FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NERASE);
1590  01e0 721b505c      	bres	20572,#5
1591                     ; 637     *pwFlash = (uint32_t)0;
1593  01e4 1e01          	ldw	x,(OFST-5,sp)
1594  01e6 a600          	ld	a,#0
1595  01e8 e703          	ld	(3,x),a
1596  01ea a600          	ld	a,#0
1597  01ec e702          	ld	(2,x),a
1598  01ee a600          	ld	a,#0
1599  01f0 e701          	ld	(1,x),a
1600  01f2 a600          	ld	a,#0
1601  01f4 f7            	ld	(x),a
1602                     ; 645 }
1605  01f5 5b08          	addw	sp,#8
1606  01f7 81            	ret
1710                     ; 656 IN_RAM(void FLASH_ProgramBlock(uint16_t BlockNum, FLASH_MemType_TypeDef FLASH_MemType, 
1710                     ; 657                         FLASH_ProgramMode_TypeDef FLASH_ProgMode, uint8_t *Buffer))
1710                     ; 658 {
1711                     	switch	.text
1712  01f8               _FLASH_ProgramBlock:
1714  01f8 89            	pushw	x
1715  01f9 5206          	subw	sp,#6
1716       00000006      OFST:	set	6
1719                     ; 659   uint16_t Count = 0;
1721                     ; 660   uint32_t startaddress = 0;
1723                     ; 663   assert_param(IS_MEMORY_TYPE_OK(FLASH_MemType));
1725                     ; 664   assert_param(IS_FLASH_PROGRAM_MODE_OK(FLASH_ProgMode));
1727                     ; 665   if(FLASH_MemType == FLASH_MEMTYPE_PROG)
1729  01fb 7b0b          	ld	a,(OFST+5,sp)
1730  01fd a1fd          	cp	a,#253
1731  01ff 260c          	jrne	L117
1732                     ; 667     assert_param(IS_FLASH_PROG_BLOCK_NUMBER_OK(BlockNum));
1734                     ; 668     startaddress = FLASH_PROG_START_PHYSICAL_ADDRESS;
1736  0201 ae8000        	ldw	x,#32768
1737  0204 1f03          	ldw	(OFST-3,sp),x
1738  0206 ae0000        	ldw	x,#0
1739  0209 1f01          	ldw	(OFST-5,sp),x
1742  020b 200a          	jra	L317
1743  020d               L117:
1744                     ; 672     assert_param(IS_FLASH_DATA_BLOCK_NUMBER_OK(BlockNum));
1746                     ; 673     startaddress = FLASH_DATA_START_PHYSICAL_ADDRESS;
1748  020d ae4000        	ldw	x,#16384
1749  0210 1f03          	ldw	(OFST-3,sp),x
1750  0212 ae0000        	ldw	x,#0
1751  0215 1f01          	ldw	(OFST-5,sp),x
1753  0217               L317:
1754                     ; 677   startaddress = startaddress + ((uint32_t)BlockNum * FLASH_BLOCK_SIZE);
1756  0217 1e07          	ldw	x,(OFST+1,sp)
1757  0219 a640          	ld	a,#64
1758  021b cd0000        	call	c_cmulx
1760  021e 96            	ldw	x,sp
1761  021f 1c0001        	addw	x,#OFST-5
1762  0222 cd0000        	call	c_lgadd
1765                     ; 680   if(FLASH_ProgMode == FLASH_PROGRAMMODE_STANDARD)
1767  0225 0d0c          	tnz	(OFST+6,sp)
1768  0227 260a          	jrne	L517
1769                     ; 683     FLASH->CR2 |= FLASH_CR2_PRG;
1771  0229 7210505b      	bset	20571,#0
1772                     ; 684     FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NPRG);
1774  022d 7211505c      	bres	20572,#0
1776  0231 2008          	jra	L717
1777  0233               L517:
1778                     ; 689     FLASH->CR2 |= FLASH_CR2_FPRG;
1780  0233 7218505b      	bset	20571,#4
1781                     ; 690     FLASH->NCR2 &= (uint8_t)(~FLASH_NCR2_NFPRG);
1783  0237 7219505c      	bres	20572,#4
1784  023b               L717:
1785                     ; 694   for(Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
1787  023b 5f            	clrw	x
1788  023c 1f05          	ldw	(OFST-1,sp),x
1790  023e               L127:
1791                     ; 696     *((PointerAttr uint8_t*) (MemoryAddressCast)startaddress + Count) = ((uint8_t)(Buffer[Count]));
1793  023e 1e0d          	ldw	x,(OFST+7,sp)
1794  0240 72fb05        	addw	x,(OFST-1,sp)
1795  0243 f6            	ld	a,(x)
1796  0244 1e03          	ldw	x,(OFST-3,sp)
1797  0246 72fb05        	addw	x,(OFST-1,sp)
1798  0249 f7            	ld	(x),a
1799                     ; 694   for(Count = 0; Count < FLASH_BLOCK_SIZE; Count++)
1801  024a 1e05          	ldw	x,(OFST-1,sp)
1802  024c 1c0001        	addw	x,#1
1803  024f 1f05          	ldw	(OFST-1,sp),x
1807  0251 1e05          	ldw	x,(OFST-1,sp)
1808  0253 a30040        	cpw	x,#64
1809  0256 25e6          	jrult	L127
1810                     ; 698 }
1813  0258 5b08          	addw	sp,#8
1814  025a 81            	ret
